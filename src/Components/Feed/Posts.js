    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator
        .share({
          title: "Hello👋",
          text:
            "Hi👋 I have shared a  new Post for " +
            Auth.Name +
      ...
      <>
        {Posts.map((post, index) => (
          ...
          >
            ...
            <Col>
              {props.showcommment === true &&
              props.showcommmentforpost === post._id ? (
                <FeedComments
                  id={post._id}
                  getposts={props.getposts}
    ...
    <>
      {isLike === false ? (
        <FcLikePlaceholder
          size={30}
          onClick={async () => {
            dispatch(likepost(props._id, Auth.Phone, props.maincode));
            setisLike(true);
