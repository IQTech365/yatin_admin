...
export default function Gift(props) {
    ...
    useEffect(async () => {
        if (MyEvents.length === 0 && myInvitations.length === 0) {
            ...
            if (
                ...
                "/MyEvents/eventpage/gift/" + props.match.params.id + "/" + props.match.params.MainCode &&
                MyEvents.length > 0
            ) {
                ...
                props.location.pathname ===
                "/inv/eventpage/gift/" + props.match.params.id + "/" + props.match.params.MainCode &&
    ...
    function getgifts(c) {
        ...
        axios.post(url + 'event/ListGifts', { category: Category })
    ...
    return (<>  <NavMobile base={base} id={props.match.params.id} />

        <Grid spacing={0} container>
            <Grid xs={12} item className="back-navigation desktop-only p-10px">
                <div style={{ width: '100%', marginTop: 5, marginRight: 3, marginLeft: 3 }}>
